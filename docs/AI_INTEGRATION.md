...
`ApiLLMAdapter` envía la pregunta al backend. El servidor comprueba que la sesión esté en `ASKING`, que el movimiento sea el actual y que la rutina siga publicada; solo entonces `GeminiQuestionService` llama `ai.interactions.create`. El modelo predeterminado es el estable `gemini-3.8-flash`, las respuestas no se almacenan en Gemini (`store: false`), el timeout predeterminado es 30 segundos y el límite local es cinco solicitudes por sesión cada diez minutos.
