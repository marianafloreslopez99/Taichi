...
TODO de integración: reemplazar `MockSpeechToTextAdapter`, `MockLLMAdapter` y `MockTextToSpeechAdapter` desde `app/services.ts`; crear cuentas e historial visible. La API actual está versionada bajo `/api/v1` y usa MySQL mediante Prisma. Las fases 7–8 requieren investigación y pruebas de precisión antes de recomendar ajustes posturales.
