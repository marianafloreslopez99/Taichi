...
describe('practice journey', () => {
  it('starts, repeats, pauses, resumes and navigates a movement', async () => {
    ...
    vi.stubGlobal('fetch', vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (path.endsWith('/routines')) return new Response(JSON.stringify({ data: routines }), { status: 200 })
      if (path.endsWith('/sessions') && init?.method === 'POST') return new Response(JSON.stringify({ data: session }), { status: 201 })
      ...
      if (action === 'previous') session = { ...session, currentMovementIndex: 0 }
      if (action === 'ask') session = { ...session, status: 'ASKING' }
      if (action === 'close-question') session = { ...session, status: 'PAUSED' }
    ...
    await user.click((await screen.findAllByRole('button', { name: /Elegir esta rutina/i }))[0]!)

    expect(
      await screen.findByRole('heading', { name: 'La apertura' }),
    ).toBeInTheDocument()
    ...
    expect(
      screen.getByRole('heading', { name: 'Manos como nubes' }),
    ).toBeInTheDocument()
