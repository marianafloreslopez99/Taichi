...
function elapsedNow(
  session: Pick<SessionRecord, 'status' | 'updatedAt' | 'elapsedSeconds'>,
) {
  if (session.status !== 'PLAYING') return session.elapsedSeconds
  return (
    session.elapsedSeconds +
    Math.max(0, Math.floor((Date.now() - session.updatedAt.getTime()) / 1000))
  )
}
...
async function saveElapsed(prisma: PrismaClient, session: SessionRecord) {
  ...
  if (elapsedSeconds === session.elapsedSeconds || session.status !== 'PLAYING')
    return session
...
export async function findRoutine(prisma: PrismaClient, routineId: string) {
  ...
  if (!routine)
    throw new HttpError(404, 'ROUTINE_NOT_FOUND', 'Rutina no encontrada.')
...
export async function createSession(
  ...
  if (movementCount === 0)
    throw new HttpError(409, 'EMPTY_ROUTINE', 'La rutina no tiene movimientos.')

  return prisma.$transaction(async (transaction) => {
    if (replaceSessionId)
      await transaction.session.deleteMany({ where: { id: replaceSessionId } })
...
export async function transitionSession(
  ...
  if (action === 'pause') {
    if (session.status !== 'PLAYING')
      throw transitionError('Solo se puede pausar una sesión en reproducción.')
    data = { status: 'PAUSED', elapsedSeconds: elapsedNow(session) }
  } else if (action === 'resume') {
    if (session.status !== 'PAUSED')
      throw transitionError('Solo se puede continuar una sesión pausada.')
    ...
    if (
      ...
      throw transitionError(
        'No se puede avanzar desde este estado o movimiento.',
      )
    }
    data = {
      currentMovementIndex: session.currentMovementIndex + 1,
      elapsedSeconds: elapsedNow(session),
    }
    ...
    if (
      ...
      throw transitionError(
        'No se puede regresar desde este estado o movimiento.',
      )
    }
    data = {
      currentMovementIndex: session.currentMovementIndex - 1,
    ...
    if (!['PLAYING', 'PAUSED'].includes(session.status))
      throw transitionError(
        'No se puede iniciar una pregunta desde este estado.',
      )
...
export async function addQuestion(
  ...
  if (session.status !== 'ASKING')
    throw transitionError('La sesión no está esperando una pregunta.')
  ...
  if (!movement)
    throw new HttpError(
      400,
      'INVALID_MOVEMENT',
      'El movimiento no pertenece a la rutina.',
    )
  await prisma.aIQuestion.create({
    data: { id: randomUUID(), sessionId, ...input },
  })
...
export async function getQuestionContext(
  ...
  const routine = await prisma.routine.findFirst({
    where: { id: session.routineId, published: true },
    include: routineInclude,
  })
  if (!routine)
    throw new HttpError(404, 'ROUTINE_NOT_FOUND', 'Rutina no encontrada.')
  ...
  const currentMovement = serialized.exercises
    .flatMap((exercise) => exercise.movements)
    .at(session.currentMovementIndex)
  if (!currentMovement || currentMovement.id !== movementId) {
    throw new HttpError(
      400,
      'INVALID_MOVEMENT',
      'La pregunta debe corresponder al movimiento actual.',
    )
  }

  return {
    sessionId,
    routineId: serialized.id,
    routineName: serialized.name,
    difficulty: serialized.difficulty,
    movementId: currentMovement.id,
    movementName: currentMovement.name,
    instruction: currentMovement.instruction,
    question,
  }
}
